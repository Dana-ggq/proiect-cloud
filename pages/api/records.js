...
//doar recomandari dintr-un oras anume
const getRecordsByCity = async (city) => {
	const collection = await getCollection(COLLECTION_NAME);
	return collection.find({ city: city }).toArray();
  }
...
//recomandari pe oras
	if (req.method === "GET" && req.query.city) {
		const city = req.query.city;
		const records = await getRecordsByCity(city);
